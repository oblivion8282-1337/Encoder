...
void OutputImage(const char* outputFileName, unsigned int width, unsigned int height, void* imageData)
{
	...
	virtual void ProcessComplete(IBlackmagicRawJob* job, HRESULT result, IBlackmagicRawProcessedImage* processedImage)
	{
		if (result == S_OK)
		{
			do
			{
				...
				OutputImage(outputFilename.c_str(), width, height, imageData);
...
{
	std::string defaultOutputFolderName = ".";
	std::string usageText = std::string("Usage: ") + argv[0] + " clipName.braw frameIndex [outputFolder (defaults to the current folder)]\n"; 
	...
	do
	{
		...
		result = codec->OpenClip(brawFilename, &clip);
		if (result != S_OK)
		{
