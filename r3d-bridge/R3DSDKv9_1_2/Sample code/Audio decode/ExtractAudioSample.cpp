...
int main(int argc, char * argv[])
{
	...
	printf("contains %u %u-bit channels at %u Hz\n", static_cast<unsigned int>(channels), static_cast<unsigned int>(samplesize), static_cast<unsigned int>(samplerate));
	...
	for (size_t al = 0U; al < blocks; al++)
	{
		...
		clip->DecodeAudioBlock(al, audiobuffer, &bufferSize);	// bufferSize gets updated
