...
/* Sample to decode the first frame and change some image processing */
...
int main(int argc, char * argv[])
{
	...
	// *** NEW IN THIS SAMPLE ***
	printf("Making the image black & white...\n");

	// the definition below sets all the image processing settings to
	// their neutral values
	ImageProcessingSettings *ip = new ImageProcessingSettings();

	// this is however not how it was seen on set so load the clip
	// settings into the object above, you will always want to do
	// this per default unless the user loads a different look or
	// has selected a different default
	clip->GetDefaultImageProcessingSettings(*ip);

	// now that we have a solid base we can change things

	// turn saturation all the way down to create a black & white image
	ip->Saturation = ImageProcessingLimits::SaturationMin;

	// and increase the contrast by 50%
	ip->Contrast = 0.5f;

	// finally let the decoder know to use the new image processing settings
